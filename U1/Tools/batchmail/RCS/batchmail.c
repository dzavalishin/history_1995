...
1.3
date	91.10.09.19.55.36;	author alex;	state Exp;
branches;
next	;
...
1.3
log
@checked in with -k by dz at 1996/09/22 04:33:50
@
text
@/*
 *
 ...
 *
 * $Log:	batchmail.c,v $
 * Revision 1.3  91/10/09  19:55:36  alex
 * Now batchmail call /usr/lib/batchuux/HOST first, then\
 * (if failed) uux. We use it to set GRADE for batchmail, for example
 * 
...
#ifndef lint
static char rcsid[] = "$Header: batchmail.c,v 1.3 91/10/09 19:55:36 alex Exp $";
#endif
...
char uux0[256];
#define UUX0            "/usr/lib/batchuux/%s"          /* uux command */
