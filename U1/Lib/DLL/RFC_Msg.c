...
hl *
RFC_headlines::have_key( const string &key ) const
    {
    hl_Cursor c( *this );
...
bool RFC_headlines::contains( const string &key ) const
    {
    return have_key( key ) ? Yes : No;
    }
...
bool RFC_headlines::
get( string &val, const string key ) const
    {
    hl *h = have_key( key );
...
time_t RFC_Msg::get_gm_time() const
    {
    string date;
    if( Yes != headline.get( date, "date" ) ) return -1;
