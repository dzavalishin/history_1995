...
@d7 7
a13 1
 *      $Log:   Q:/lib/fido/vcs/fido_msg.h_v  $
d130 1
a130 1
class	fido_msg
d227 2
a228 2
	void	get_subj( uchar *s )				{ strncpy( s, hdr.subject, 72 ); }
	void	set_subj( const uchar *s )			{ strncpy( hdr.subject, s, 72 ); hdr.subject[71] = '\0'; flush(); }
d230 2
a231 2
	void	get_fsc35_reply_addr( uchar *buf )			{	strcpy( buf, fsc35_reply_addr );	}
	void	set_fsc35_reply_addr( const uchar *buf )	{	strncpy( fsc35_reply_addr, buf, FSC35RA ); fsc35_reply_addr[FSC35RA-1] = '\0';	}
d247 1
a247 1
int	find_last_msg( const char *dir );
d251 1
a251 1
#endif	FIDO_MSG_H
@
