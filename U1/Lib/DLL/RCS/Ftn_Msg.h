...
//    bool          hflush;                 // Header must be flushed
    bool          rewrite;                // Message must be written down on detach
...
// Message header access
    ...
    void          set_attr( const fmsg_attr a )        { flags = a; rewrite = Yes; }
    void          add_attr( const fmsg_attr a )        { flags |= a; rewrite = Yes; }
    void          del_attr( const fmsg_attr a )        { flags &= ~a; rewrite = Yes; }
    ...
    void          set_subj( const string &s )            { strncpy( hdr.subject, s, 71 ); hdr.subject[71] = '\0'; rewrite = Yes; }

    void          get_fsc35_reply_addr( string &buf )            { buf = fsc35_reply_addr; }
    void          set_fsc35_reply_addr( const string &buf )    {  fsc35_reply_addr = buf; rewrite = Yes; }

    void          get_fsc35_reply_to( fido_user &fa )            {    fa = fsc35_reply_to;    }
    void          set_fsc35_reply_to( const fido_user &fa )    {    fsc35_reply_to = fa;  rewrite = Yes;  }

    void          set_via_addr( const string &a ) { via_addr = a; rewrite = Yes; }
