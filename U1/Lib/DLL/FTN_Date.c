...
void
FTN_Msg::set_date( long tmt )
    {
    char    buf[40];
    
    if( tmt == -1 )
        {
        ::error( EI_None, "FTN_Msg::set_date( %ld ), reverting to time()", tmt );
        tmt = time(NULL);
        }
...
void
FTN_Msg::set_date( const string &d )
    {
    sprintf( hdr.date,	"%.19s", (const char *)d );
    rewrite = Yes;
    }
