...
bool ftn::add_def( const char *def )	// Parse definition
    {
...
bool ftn::valid()					// Can we continue?
    {
    return ftn_count == 0 ? No : Yes;
    }


const ftn_def & ftn::def()
    {
    if( !valid() )
...
const fido_addr ftn::match_aka( const fido_addr &his_a )
    {
    ftn_def	d;
...
bool ftn::match( ftn_def &out, const fido_addr &his_a )
    {
    if( !valid() )
        fatal( EC_Incorrect_Setup, EI_None, "No FTNs defined" );
...
static bool uu_traverse( uucp_addr::el_type t, const char *el, int len, void *arg )
    {
...
bool ftn::match( ftn_def &out, const uucp_addr &in )
    {
    if( !valid() )
        fatal( EC_Incorrect_Setup, EI_None, "No FTNs defined" );
...
bool ftn::find( ftn_def &out, const fido_addr &in )
    {
    for( int i = 0; i < ftn_count; i++ )
        {
