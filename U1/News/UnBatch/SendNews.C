 ...
 *
 *      $Log: SendNews.C $
 *      Revision 1.35  1995/08/07 14:55:01  dz
 *      Using class App
 *
...
UnbatchApp::packetize_news( VReadBase &in, uuinfo &ui, echo_def &edef,
	...
	if( !edef.pass_to_fido() )
		{
		if( verbose ) error( EI_None, "Group %s isn't allowed to be passed to FIDO", (const char *) edef.group().c_str() );
                else
                    {
                    fprintf( stderr, "?" );
                    fflush( stderr );
	...
	if( verbose ) fprintf( stderr, "Echo: %s\n", (const char *)edef.area().c_str() );
        else          
            {
            fprintf( stderr, "." );
            fflush( stderr );
